    ...
    public sealed class SolitaireDragPresenter
    {
        ...
        public void BeginDrag(int pressedCardId, Vector3 pointerDownWorld, Transform dragParent)
        {
            ...
            for (int i = 0; i < _draggedCount; i++)
            {
                ...
                CardInputReceiver receiver = card.GetComponent<CardInputReceiver>();
                receiver.DragBehaviour.BeginDrag(dragParent, pointerDownWorld);
                card.SetSortingOrder(_config.DragSortingOrder + i);
                card.BeginDragVisual(_config);
            }
        ...
        public void MoveDraggedCards(Vector3 pointerWorld)
        {
            ...
            {
                CardView card = _context.ViewRegistry.GetCard(_draggedCardIds[i]);
                card.GetComponent<CardInputReceiver>().DragBehaviour.MoveToPointer(pointerWorld);
            }
        }
        ...
        public void FinishDrag()
        {
            for (int i = 0; i < _draggedCount; i++)
            {
                CardView card = _context.ViewRegistry.GetCard(_draggedCardIds[i]);
                CardInputReceiver receiver = card.GetComponent<CardInputReceiver>();
                receiver.DragBehaviour.EndDrag(card.CachedTransform.parent);
                card.EndDragVisual();
            }
